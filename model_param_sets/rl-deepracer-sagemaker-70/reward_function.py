...
class TrackSegments(LinearRing):
    __slots__ = 'segments'
    ...
    def __init__(self, waypoints):
        ...
        ring = tuple((wp.x, wp.y) for wp in waypoints)
        super().__init__(ring)
...
class Simulation:
    __slots__ = 'sim_state_initialized', 'run_state', 'timer', 'progress_advancements', 'track_waypoints', 'track_segments', 'central_track_ring'
    ...
    def __init__(self):
        ...
        self.central_track_ring = None

    def initialize_sim_state(self, params):
        self.track_waypoints = TrackWaypoints(params['waypoints'])
        self.track_segments = TrackSegments(self.track_waypoints.waypoints)
        self.sim_state_initialized = True

    def add_run_state(self, params):
        if not self.sim_state_initialized:
            self.initialize_sim_state(params)
...
class RunState:
    ...
    def additive_factors(self):
        return self.center_line_reward, self.waypoint_heading_reward

    @property
    def multiplicative_factors(self):
        return self.next_speed_reward, self.progress_reward, self.steering_reward
    ...
    def target_steering_reward(self):
        if self.prev_run_state:
            prev_steering_diff = self.prev_run_state.target_steering_angle - self.prev_run_state.steering_angle
            if prev_steering_diff < 0:
                if self.steering_angle < self.prev_run_state.steering_angle:
                    bonus = 1.1
                else:
                    bonus = 0.9
            elif prev_steering_diff > 0:
                if self.steering_angle > self.prev_run_state.steering_angle:
                    bonus = 1.1
                else:
                    bonus = 0.9
        ...
        if abs_steering_diff == 0:
            return 1
        return max(math.cos(math.radians(abs_steering_diff)) * bonus / 1.1, MIN_REWARD)

    @property
    def target_steering_angle(self):
        if self.next_segment_approaching_large_curve:
    ...
    @property
    def next_segment_approaching_large_curve(self):
        return self.next_segment_approaching and self.large_curve_ahead
